...
const framesFeed = document.querySelector(".frames-feed");
const framesCommentsModal = document.querySelector(".frames-comments-modal");
const framesCommentsClose = document.querySelector(".frames-comments-close");
const framesCommentsList = document.querySelector(".frames-comments-list");
const framesCommentForm = document.querySelector(".frames-comment-form");
const framesEndBubble = document.querySelector(".frames-end-bubble");
const notificationsFeed = document.querySelector(".notifications-feed");
const homePage = document.querySelector(".home-page");
const framesPage = document.querySelector(".frames-page");
...
const tripRouteWave = 16;

const staticFrames = [
  {
    id: "frame-1",
    title: "Jak korzystać z Bloga: podstawowe funkcje",
    videoSrc: "./1.mov",
    authorName: "Kacper Czarnojan",
    likes: {},
    dislikes: {},
    comments: [],
...
const localFramesKey = "wakacje-z-bogiem-frames";
...
let routeBusShouldDriveIn = false;
let routeBusScrollFrame = 0;
let routeBusScrollTimer = 0;
let routeBusFloatTimer = 0;
let routeBusDriveFrame = 0;
let routeBusDriveTimer = 0;
let openFrameCommentsId = "";
let openFramePanelMode = "";
let framesScrollFrame = 0;
let framesEndBubbleTimer = 0;
let framesTouchStartY = 0;
...
const routeBeepSound = new Audio("./beep.mp3");
routeBeepSound.preload = "auto";
routeBeepSound.volume = 0.46;
...
const playRouteBeepSound = () => {
  try {
    if (!routeBeepSound.paused && !routeBeepSound.ended) {
      return;
    }

    routeBeepSound.currentTime = 0;
    const playPromise = routeBeepSound.play();

    if (playPromise?.catch) {
      playPromise.catch(() => {});
    }
  } catch {
    // Brak pliku dzwieku trasy nie powinien blokowac przejscia.
  }
};
...
const getRouteBusDriveDuration = (busPosition) => {
  const distance = Math.max(0, busPosition.y - tripRouteTopOffset);
  return Math.round(Math.max(3800, Math.min(11000, distance * 7)));
};

const cancelRouteBusFollowScroll = () => {
  window.clearTimeout(routeBusScrollTimer);
  window.clearTimeout(routeBusDriveTimer);
  window.cancelAnimationFrame(routeBusScrollFrame);
  window.cancelAnimationFrame(routeBusDriveFrame);
};

const findRoutePathLengthAtY = (path, targetY) => {
...
const scrollRouteBusIntoView = (timeline, point) => {
  if (point.y < getRoutePoint(2).y - 4) {
    return;
  }
  ...
  const targetY = Math.max(
    ...
    Math.min(
      ...
      timelineTop + point.y - window.innerHeight * 0.52
    )
  );
...
const driveRouteBusAlongPath = (targetPosition, duration) => {
  cancelRouteBusFollowScroll();

  const timeline = routeContent?.querySelector(".route-timeline");
  const path = routeContent?.querySelector(".route-path-line");
  const bus = routeContent?.querySelector("[data-route-status]");

  if (!timeline || !path || !bus) {
    return;
  }

  const targetLength = findRoutePathLengthAtY(path, targetPosition.y);
  const delay = 500;

  routeBusDriveTimer = window.setTimeout(() => {
    ...
    const step = (now) => {
      ...
      const point = path.getPointAtLength(targetLength * eased);

      bus.style.setProperty("--route-bus-x", `${(point.x - tripRouteCenterX).toFixed(2)}px`);
      bus.style.setProperty("--route-bus-y", `${point.y.toFixed(2)}px`);
      scrollRouteBusIntoView(timeline, point);

      if (progress < 1 && appScreen.dataset.view === "route" && bus.isConnected) {
        routeBusDriveFrame = window.requestAnimationFrame(step);
        return;
      }

      const finalPoint = path.getPointAtLength(targetLength);
      bus.style.setProperty("--route-bus-x", `${(finalPoint.x - tripRouteCenterX).toFixed(2)}px`);
      bus.style.setProperty("--route-bus-y", `${finalPoint.y.toFixed(2)}px`);
      bus.classList.remove("is-driving-in");
    };
...
const getRoutePathPointAtY = (targetY) => {
  const path = document.createElementNS("http://www.w3.org/2000/svg", "path");
  path.setAttribute("d", buildRoutePathData());
  const length = findRoutePathLengthAtY(path, targetY);
...
const getRouteBusPosition = (date = getRouteActiveDate()) => {
  ...
  return getRoutePathPointAtY(from.y + (to.y - from.y) * segmentProgress);
};
...
const getRouteStopDate = (stop) =>
  new Date(tripRouteStartDate.getTime() + (stop.day - 1) * tripRouteDayMs);

const getRouteStopBubbleCopy = (stop, date = getRouteActiveDate()) => {
  const routeDate = getRouteStopDate(stop);
  const currentDay = new Date(date.getFullYear(), date.getMonth(), date.getDate());
  const routeDay = new Date(routeDate.getFullYear(), routeDate.getMonth(), routeDate.getDate());
  const dayDiff = Math.ceil((routeDay.getTime() - currentDay.getTime()) / tripRouteDayMs);
  ...
  return `${Math.abs(dayDiff)} dni temu`;
};
...
const buildRouteClockIcon = () => `
  <svg class="route-card-icon route-card-clock" viewBox="0 0 24 24" aria-hidden="true">
    <circle cx="12" cy="12" r="8.2" />
    <path d="M12 7.8v4.55l3.05 1.8" />
  </svg>
`;
...
const buildRouteMarkerMarkup = (stop, index, activeDate) => {
  const point = getRoutePoint(index);
  return `
    <button
      class="route-marker-button"
      type="button"
      data-route-marker
      aria-label="${stop.place}: ${getRouteStopBubbleCopy(stop, activeDate)}"
      style="--route-marker-x:${(point.x - tripRouteCenterX).toFixed(2)}px; --route-marker-y:${point.y.toFixed(2)}px;"
    >
      <span class="route-marker-halo" aria-hidden="true"></span>
      <span class="route-marker-dot" aria-hidden="true"></span>
      <span class="route-marker-bubble" role="status">${getRouteStopBubbleCopy(stop, activeDate)}</span>
    </button>
  `;
};
...
const buildRouteStopMarkup = (stop, index) => {
  ...
  const cardMarkup = `
    <div class="route-info-card">
      ...
      <span class="route-card-row route-day-row">
        ${buildRouteClockIcon()}
  ...
  return `
    <article class="route-stop ${isPhotoLeft ? "photo-left" : "photo-right"}" style="top:${point.y - 70}px;">
...
const renderRoute = () => {
  ...
  const shouldDriveIn = routeBusShouldDriveIn && busPosition.y > tripRouteTopOffset + 8;
  const busDriveDuration = shouldDriveIn ? getRouteBusDriveDuration(busPosition) : 0;
  const renderBusPosition = shouldDriveIn
    ? { x: tripRouteCenterX, y: tripRouteTopOffset }
    : busPosition;
  const stopsMarkup = tripRouteStops.map(buildRouteStopMarkup).join("");
  const markerMarkup = tripRouteStops
    .map((stop, index) => buildRouteMarkerMarkup(stop, index, activeDate))
    .join("");
  ...
  routeContent.innerHTML = `
    ...
    <div
      ...
      style="--route-height:${height}px; --route-bus-x:${(renderBusPosition.x - tripRouteCenterX).toFixed(2)}px; --route-bus-y:${renderBusPosition.y.toFixed(2)}px; --route-bus-drive-duration:${busDriveDuration}ms;"
    >
      ${buildRouteSvg()}
      ${markerMarkup}
      <button class="route-moving-bus${shouldDriveIn ? " is-driving-in" : ""}" type="button" data-route-status aria-label="${getRouteStatusCopy(activeDate)}">
        ${buildBusIcon()}
  ...
  routeBusShouldDriveIn = false;
  window.clearTimeout(routeBusFloatTimer);
  if (shouldDriveIn) {
    driveRouteBusAlongPath(busPosition, busDriveDuration);
    routeBusFloatTimer = window.setTimeout(() => {
      routeContent.querySelector("[data-route-status]")?.classList.remove("is-driving-in");
    }, busDriveDuration + 650);
  }
...
document.addEventListener(
  ...
  (event) => {
    ...
    if (button) {
      ...
      if (button.closest("[data-route-status]")) {
        return;
      }
...
const isInteractive = (target) =>
  Boolean(target.closest("button, input, label, form, .auth-panel, .compose-panel, .panel-option, .users-list, .post-card, .frame-card, .frames-comments-modal"));

routeContent?.addEventListener("click", (event) => {
  const routePhotoDot = event.target.closest("[data-route-photo-dot]");
  const routeMarker = event.target.closest("[data-route-marker]");
  ...
  if (routeMarker) {
    routeMarker.classList.add("is-hint-open");
    window.clearTimeout(routeMarker.hintTimer);
    routeMarker.hintTimer = window.setTimeout(() => {
      routeMarker.classList.remove("is-hint-open");
    }, 2600);
    return;
  }
...
routeContent?.addEventListener("pointerover", (event) => {
  const button = event.target.closest("[data-route-status]");
...
const isCloseIcon = (target) =>
  Boolean(target.closest(".close-icon, .profile-close-icon, .compose-close-icon"));
...
const moveDockIndicator = (index, { animateDock = true, animateView = true } = {}) => {
  ...
  const nextView =
    clampedIndex === 3 ? "notifications" :
    clampedIndex === 2 ? "route" :
    clampedIndex === 1 ? "frames" :
    "home";
  const previousView = appScreen.dataset.view || "home";
  appScreen.dataset.view = nextView;
  homePage.setAttribute("aria-hidden", String(nextView !== "home"));
  framesPage?.setAttribute("aria-hidden", String(nextView !== "frames"));
...
const readLocalFrames = () => {
  try {
    return JSON.parse(localStorage.getItem(localFramesKey) || "[]");
  } catch {
    return [];
  }
};
...
const writeLocalFrames = (frames) => {
  localStorage.setItem(localFramesKey, JSON.stringify(frames));
};

const getFrameVideoSource = async (frame) => {
  return frame?.videoSrc || frame?.src || "";
};
...
const fetchFirebaseFrames = async () => {
  if (useLocalDatabase || !firebaseBaseUrl) {
    return null;
  }

  const response = await fetch(`${firebaseBaseUrl}/wakacjeZBogiem/frames.json`);
  if (!response.ok) {
    throw new Error("Nie udalo sie pobrac kadrow.");
  }
...
const saveFirebaseFrame = async (frame) => {
  if (useLocalDatabase || !firebaseBaseUrl) {
    return false;
  }

  const response = await fetch(`${firebaseBaseUrl}/wakacjeZBogiem/frames/${frame.id}.json`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
  ...
  if (!response.ok) {
    throw new Error("Nie udalo sie zapisac kadru.");
  }
...
const frameStorageData = (frame = {}) => ({
  id: frame.id,
  likes: frame.likes || {},
  dislikes: frame.dislikes || {},
  comments: frame.comments || [],
  updatedAt: new Date().toISOString(),
});

const mergeStaticFrames = (storedFrames = []) => {
  const storedMap = new Map(storedFrames.map((frame) => [frame.id, frame]));

  return staticFrames.map((frame) => {
    const storedFrame = storedMap.get(frame.id) || {};

    return {
      ...frame,
      likes: storedFrame.likes || frame.likes || {},
      dislikes: storedFrame.dislikes || frame.dislikes || {},
      comments: storedFrame.comments || frame.comments || [],
    };
  });
};
...
const getFrames = async () => {
  try {
    const firebaseFrames = await fetchFirebaseFrames();
    if (firebaseFrames) {
      return mergeStaticFrames(firebaseFrames);
    }
  } catch {
    // Lokalny fallback pozwala ogladac kadry z plikow nawet bez stabilnej chmury.
  }

  return mergeStaticFrames(readLocalFrames());
};

const saveFrame = async (frame) => {
  const storedFrame = frameStorageData(frame);

  if (await saveFirebaseFrame(storedFrame).catch(() => false)) {
    return;
  }

  const frames = readLocalFrames().filter((item) => item.id !== storedFrame.id);
  frames.push(storedFrame);
  writeLocalFrames(frames);
};
...
const isAuthorUser = (user) =>
  Boolean(user?.isAuthor) || normalize(`${user?.firstName || ""} ${user?.lastName || ""}`) === "kacper czarnojan";
...
const setPostSocialPanel = (postId, type, isOpen = true) => {
  ...
  modal.classList.remove("is-comments", "is-likes", "is-dislikes");
  modal.classList.add(`is-${type}`);
  title.textContent = type === "likes" ? "Polubili post" : type === "dislikes" ? "Łapki w dół" : "Komentarze";
...
const renderPosts = async ({ animateSocialPanel = true } = {}) => {
  ...
  posts.forEach((post, postIndex) => {
    ...
    modalTitle.textContent = isLikesOpen ? "Polubili post" : openType === "dislikes" ? "Łapki w dół" : "Komentarze";
    ...
    dislikesList.className = "dislikes-list";
    const dislikeNames = likeNamesFor(post.dislikes);
    if (!dislikeNames.length) {
      const empty = document.createElement("span");
      empty.className = "social-empty";
      empty.textContent = "Nikt nie dał łapki w dół.";
      dislikesList.append(empty);
    } else {
      dislikeNames.forEach((nameText) => {
        const item = document.createElement("span");
        item.className = "like-person";
        item.textContent = nameText;
        dislikesList.append(item);
      });
    }
    ...
    socialModal.append(likesList, dislikesList, comments, commentForm);
...
const frameReactionCount = (map = {}) => Object.keys(map || {}).length;

const createFrameActionButton = ({ type, count, frameId, isActive = false }) => {
  const button = document.createElement("button");
  button.className = `frame-action-button is-${type}${isActive ? " is-active" : ""}`;
  button.type = "button";
  button.dataset.frameAction = type;
  ...
  const paths = {
    like: [
      "M12 20.35S4.9 16.1 3.2 10.95C2.1 7.58 4.1 4.8 7.1 4.8c1.72 0 3.18.94 4.02 2.28C11.96 5.74 13.42 4.8 15.14 4.8c3 0 5 2.78 3.9 6.15C17.35 16.1 12 20.35 12 20.35Z",
    ],
    dislike: [
      "M7.2 3.8h8.1c.87 0 1.62.58 1.84 1.42l1.23 4.68c.3 1.16-.57 2.3-1.77 2.3h-3.08l.52 3.45c.13.86-.16 1.73-.78 2.34L12.1 19.1 7.7 12.2H4.6V3.8h2.6Z",
      "M7.7 4v8.2",
    ],
    comment: [
      "M5.2 5.4h13.6a2.5 2.5 0 0 1 2.5 2.5v6.15a2.5 2.5 0 0 1-2.5 2.5H10.2L5.1 20v-3.45A2.5 2.5 0 0 1 2.7 14.05V7.9a2.5 2.5 0 0 1 2.5-2.5Z",
    ],
  };

  paths[type].forEach((value) => {
    const path = document.createElementNS("http://www.w3.org/2000/svg", "path");
...
const closeFrameComments = () => {
  openFrameCommentsId = "";
  openFramePanelMode = "";
  if (framesCommentsModal) {
    framesCommentsModal.hidden = true;
    framesCommentsModal.dataset.frameId = "";
    framesCommentsModal.dataset.frameMode = "";
  }
};

const renderFramePeopleModal = (frame, type) => {
  if (!framesCommentsModal || !framesCommentsList || !framesCommentForm) {
    return;
  }

  const isLikes = type === "likes";
  openFrameCommentsId = frame.id;
  openFramePanelMode = type;
  framesCommentsModal.hidden = false;
  framesCommentsModal.dataset.frameId = frame.id;
  framesCommentsModal.dataset.frameMode = type;
  framesCommentsList.innerHTML = "";
  framesCommentForm.hidden = true;
  framesCommentsModal.querySelector(".frames-comments-title").textContent = isLikes ? "Polubili film" : "Łapki w dół";

  const names = likeNamesFor(isLikes ? frame.likes : frame.dislikes);
  if (!names.length) {
    const empty = document.createElement("span");
    empty.className = "frames-comments-empty";
    empty.textContent = isLikes ? "Jeszcze nikt nie polubił tego filmu." : "Nikt nie dał łapki w dół.";
    framesCommentsList.append(empty);
    return;
  }

  names.forEach((nameText) => {
    const row = document.createElement("div");
    row.className = "frame-comment-row";
    const author = document.createElement("strong");
    author.textContent = nameText;
    row.append(author);
    framesCommentsList.append(row);
  });
};

const renderFrameCommentsModal = (frame) => {
  if (!framesCommentsModal || !framesCommentsList || !framesCommentForm) {
    return;
  }

  openFrameCommentsId = frame.id;
  openFramePanelMode = "comments";
  framesCommentsModal.hidden = false;
  framesCommentsModal.dataset.frameId = frame.id;
  framesCommentsModal.dataset.frameMode = "comments";
  framesCommentsList.innerHTML = "";
  framesCommentForm.hidden = false;
  framesCommentsModal.querySelector(".frames-comments-title").textContent = "Komentarze";

  const comments = frame.comments || [];
  if (!comments.length) {
    const empty = document.createElement("span");
    empty.className = "frames-comments-empty";
    empty.textContent = "Jeszcze nie ma komentarzy.";
    framesCommentsList.append(empty);
  } else {
    comments.forEach((comment) => {
      ...
      row.className = "frame-comment-row";
      const author = document.createElement("strong");
      author.textContent = comment.userName || "Użytkownik";
      const text = document.createElement("span");
      text.textContent = comment.text || "";
  ...
  const input = framesCommentForm.elements.comment;
  input.disabled = !readCurrentUser();
  input.placeholder = input.disabled ? "Zaloguj się, aby komentować" : "Napisz komentarz...";
  framesCommentForm.querySelector("button").disabled = input.disabled;
};

const showFramesEndBubble = () => {
  if (!framesEndBubble) {
    return;
  }

  framesEndBubble.classList.add("is-visible");
  framesEndBubble.setAttribute("aria-hidden", "false");
  window.clearTimeout(framesEndBubbleTimer);
  framesEndBubbleTimer = window.setTimeout(() => {
    framesEndBubble.classList.remove("is-visible");
    framesEndBubble.setAttribute("aria-hidden", "true");
  }, 2400);
};
...
const isAtLastFrame = () => {
  ...
  return framesFeed.scrollTop + framesFeed.clientHeight >= framesFeed.scrollHeight - 4;
};

const syncVisibleFrameVideo = () => {
  if (!framesFeed) {
    return;
  }

  const feedRect = framesFeed.getBoundingClientRect();
  const feedCenter = feedRect.top + feedRect.height / 2;
  let closestVideo = null;
  let closestDistance = Infinity;

  framesFeed.querySelectorAll(".frame-video").forEach((video) => {
    const rect = video.closest(".frame-card")?.getBoundingClientRect();
    if (!rect) {
      return;
    }
  ...
  framesFeed.querySelectorAll(".frame-video").forEach((video) => {
    if (video === closestVideo && appScreen.dataset.view === "frames") {
      video.muted = false;
      video.volume = 1;
...
const renderFrames = async () => {
  ...
  const frames = sortFrames(await getFrames());
  const user = readCurrentUser();
  framesFeed.innerHTML = "";

  if (!frames.length) {
    const empty = document.createElement("p");
    empty.className = "frames-empty";
    empty.textContent = "Tu pojawią się pierwsze Kadry.";
    framesFeed.append(empty);
    closeFrameComments();
    return;
  }
  ...
  for (const [frameIndex, frame] of frames.entries()) {
    ...
    card.style.setProperty("--item-index", String(Math.min(frameIndex, 7)));

    const videoSrc = await getFrameVideoSource(frame);
    if (videoSrc) {
      ...
      missing.className = "frames-empty";
      missing.textContent = "Ten film jest zapisany tylko lokalnie na urządzeniu, na którym został dodany.";
      card.append(missing);
    }
    ...
    actions.append(
      createFrameActionButton({
        type: "like",
        count: frameReactionCount(frame.likes),
        frameId: frame.id,
        isActive: Boolean(user && frame.likes?.[user.id]),
      }),
      createFrameActionButton({
        type: "dislike",
        count: frameReactionCount(frame.dislikes),
        frameId: frame.id,
        isActive: Boolean(user && frame.dislikes?.[user.id]),
      }),
      createFrameActionButton({
        type: "comment",
        count: (frame.comments || []).length,
        frameId: frame.id,
      })
    );
    ...
    title.textContent = frame.title || "Kadr z wyjazdu";

    card.append(actions, title);
    framesFeed.append(card);
  }

  const openedFrame = frames.find((frame) => frame.id === openFrameCommentsId);
  if (openedFrame) {
    if (openFramePanelMode === "likes" || openFramePanelMode === "dislikes") {
      renderFramePeopleModal(openedFrame, openFramePanelMode);
    } else {
      renderFrameCommentsModal(openedFrame);
    }
  } else {
    closeFrameComments();
  }
  ...
  window.setTimeout(scheduleDockContrast, 0);
  window.setTimeout(syncVisibleFrameVideo, 80);
};
...
const buildNotifications = async (user) => {
  ...
  posts.forEach((post) => {
    ...
    if (postCreatedAt >= accountCreatedAt) {
      items.push({
        ...
        title: "Nowy opublikowany wpis na Stronie Głównej",
        detail: `„${post.title}”`,
      });
    }
  ...
  frames.forEach((frame) => {
    const frameCreatedAt = new Date(frame.createdAt).getTime();
    if (frameCreatedAt >= accountCreatedAt) {
      items.push({
        id: `frame:${frame.id}`,
        createdAt: frame.createdAt,
        type: "frame",
        actor: frame.authorName || "Kacper Czarnojan",
        title: "Nowy Film na Blogu",
        detail: `W zakładce Kadry: „${frame.title || "Kadr z wyjazdu"}”.`,
      });
    }
  });

  const now = new Date();
  tripRouteStops.forEach((stop) => {
    const stopDate = getRouteStopDate(stop);
    const stopTime = stopDate.getTime();

    if (stopTime > now.getTime() || stopTime < accountCreatedAt) {
      return;
    }

    const isFirstDay = stop.day === 1;
    items.push({
      id: `route:${stop.day}`,
      createdAt: stopDate.toISOString(),
      type: "route",
      actor: "Trasa",
      title: isFirstDay ? "Zaczęliśmy podróż" : "Następny dzień wycieczki",
      detail: isFirstDay
        ? "Wyruszamy w trasę. Pierwszy etap: Przejazd (Polska → Serbia)."
        : `Dzisiaj zmierzamy do: ${stop.place}.`,
    });
  });
...
const setLoggedView = (user) => {
  ...
  const sessionUser = isLogged && isAuthorUser(user) && !user.isAuthor
    ? { ...user, isAuthor: true }
    : user;
  ...
  if (isLogged) {
    ...
    applyAppSettings(getUserAppSettings(sessionUser));
    profileMorph.removeAttribute("data-auth-mode");
    profileSettingsTrigger.hidden = false;
    profileSettingsPanel.hidden = false;
    setProfileSettingsOpen(false);
    loggedName.textContent = `Witaj na blogu, ${sessionUser.firstName}.`;
    authMessage.textContent = "";
    updateAuthorUi();
    renderPosts();
    renderFrames();
...
framesFeed?.addEventListener("click", async (event) => {
  ...
  const frameId = actionButton.dataset.frameId;
  ...
  if (action === "comment") {
    renderFrameCommentsModal(frame);
    return;
  }
  ...
  frame.likes = frame.likes || {};
  frame.dislikes = frame.dislikes || {};
  const targetMap = action === "like" ? frame.likes : frame.dislikes;
  const oppositeMap = action === "like" ? frame.dislikes : frame.likes;

  if (targetMap[user.id]) {
    delete targetMap[user.id];
  } else {
    targetMap[user.id] = currentUserDisplay();
    delete oppositeMap[user.id];
    if (action === "like") {
      playHeartSound();
    }
  }

  const card = actionButton.closest(".frame-card");
  const likeButton = card?.querySelector("[data-frame-action='like']");
  const dislikeButton = card?.querySelector("[data-frame-action='dislike']");
  likeButton?.classList.toggle("is-active", Boolean(frame.likes[user.id]));
  dislikeButton?.classList.toggle("is-active", Boolean(frame.dislikes[user.id]));
  if (likeButton) {
    likeButton.querySelector("span").textContent = String(frameReactionCount(frame.likes));
  }
  if (dislikeButton) {
    dislikeButton.querySelector("span").textContent = String(frameReactionCount(frame.dislikes));
  }

  if (isAuthorUser(user)) {
    renderFramePeopleModal(frame, action === "like" ? "likes" : "dislikes");
  }

  await saveFrame(frame);
});

framesFeed?.addEventListener("scroll", () => {
  window.cancelAnimationFrame(framesScrollFrame);
  framesScrollFrame = window.requestAnimationFrame(syncVisibleFrameVideo);
}, { passive: true });

framesFeed?.addEventListener("wheel", (event) => {
  if (event.deltaY > 0 && isAtLastFrame()) {
    event.preventDefault();
    showFramesEndBubble();
  }
}, { passive: false });

framesFeed?.addEventListener("touchstart", (event) => {
  framesTouchStartY = event.touches[0]?.clientY || 0;
}, { passive: true });

framesFeed?.addEventListener("touchmove", (event) => {
  const nextY = event.touches[0]?.clientY || framesTouchStartY;
  if (framesTouchStartY - nextY > 16 && isAtLastFrame()) {
    event.preventDefault();
    showFramesEndBubble();
  }
}, { passive: false });

framesCommentsClose?.addEventListener("click", (event) => {
  event.preventDefault();
  event.stopPropagation();
  closeFrameComments();
});

framesCommentForm?.addEventListener("submit", async (event) => {
  event.preventDefault();
  event.stopPropagation();
  const user = readCurrentUser();
  const canInspectReactions = isAuthorUser(user);
  ...
  const text = framesCommentForm.elements.comment.value.trim();
  const frameId = framesCommentsModal?.dataset.frameId;
...
postsFeed.addEventListener("click", async (event) => {
  ...
  if (heartButton?.dataset.likeTarget === "post") {
    openPostPanel = canInspectReactions ? { postId: heartButton.dataset.postId, type: "likes" } : null;
    pendingDeletePostId = null;
    if (canInspectReactions) {
  ...
  if (dislikeButton) {
    if (canInspectReactions) {
      openPostPanel = { postId: dislikeButton.dataset.postId, type: "dislikes" };
      pendingDeletePostId = null;
      setPostSocialPanel(dislikeButton.dataset.postId, "dislikes", true);
    } else {
      openPostPanel = null;
    }
  ...
  if (heartButton?.dataset.likeTarget === "comment") {
    ...
    openPostPanel = canInspectReactions ? { postId: post.id, type: "likes" } : null;
    pendingDeletePostId = null;
  } else {
    post.dislikes = post.dislikes || {};
    post.likes = post.likes || {};
    ...
    openPostPanel = canInspectReactions ? { postId: post.id, type: "dislikes" } : null;
    pendingDeletePostId = null;
  }
...
const handleDockAutoCollapse = () => {
  ...
  const canAutoCollapse = appScreen.dataset.view !== "notifications" && appScreen.dataset.view !== "frames";
  const hasOpenPanel =
    appScreen.classList.contains("menu-open") ||
    appScreen.classList.contains("profile-open") ||
    appScreen.classList.contains("compose-open");

  if (!canAutoCollapse || hasOpenPanel || currentScrollTop < dockExpandThreshold) {
    expandDock();
